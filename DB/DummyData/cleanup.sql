...
DELETE FROM CLIENT_IN_ROOMS WHERE allocated_room_id = any ((SELECT distinct(id) FROM rooms_allocation
MINUS
SELECT distinct(allocated_room_id) FROM CLIENT_IN_ROOMS )
UNION
(SELECT distinct(id) FROM rooms_allocation MINUS SELECT allocated_room_id FROM BOOKING_ROOMS_ALLOCATION)) ;

DELETE FROM BOOKING_ROOMS_ALLOCATION WHERE allocated_room_id = any ((SELECT distinct(id) FROM rooms_allocation
MINUS
SELECT distinct(allocated_room_id) FROM CLIENT_IN_ROOMS )
UNION
(SELECT distinct(id) FROM rooms_allocation MINUS SELECT allocated_room_id FROM BOOKING_ROOMS_ALLOCATION)) ;
